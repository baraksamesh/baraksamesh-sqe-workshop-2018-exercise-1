...
const functionHandler = (jsonObject, table) =>{
    ...
    row.push('');
    table.push(row);
    jsonObject.params.forEach(param => {paramHandler(param, table);});
    jsonObject.body.body.forEach(statement => {jsonToArray(statement, table);});
    return table;
};
...
const ifHandler = (jsonObject, table) =>{
    ...
    jsonObject.consequent.body === undefined ? jsonToArray(jsonObject.consequent, table):
        jsonObject.consequent.body.forEach(statement => {jsonToArray(statement, table);});
    if(jsonObject.alternate!==null){
        ...
        jsonObject.alternate.body === undefined ? jsonToArray(jsonObject.alternate, table):
            jsonObject.alternate.body.forEach(statement => {jsonToArray(statement, table);});
    }
    return table;
};
